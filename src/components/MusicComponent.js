...
function MusicComponent({
  ...
  return (
    <>
      <div
        style={MaxWindow ? { backgroundImage: `url(${songImg})` } : null}
        className={MaxWindow ? "music-container-1" : "music-container"}
      >
        <Slider
          className={MaxWindow ? "audio-input-1" : "audio-input"}
        ...
        <div className={MaxWindow ? "music-parts-1" : "music-parts"}>
          <audio
            ref={audioRef}
            className={MaxWindow ? "audio-element-1" : "audio-element"}
          >
            <source src={songAudio}></source>
          </audio>
          <div className={MaxWindow ? "img-container-1" : "img-container"}>
            <img
              className={MaxWindow ? "img-1" : "img"}
              src={songImg}
              alt="hello"
            ></img>
            <div className="hover-icon" onClick={handleMaxWindow}>
              <MyCustomMaximizeIcon
                style={{ fontSize: "24px", color: "white" }}
              />
            </div>
            {MaxWindow ? (
              <div className="goback" onClick={handleMinWindow}>
                <MyCustomGoBackIcon />
              </div>
            ) : null}
          </div>
          <div
            className={MaxWindow ? "detail-container-1" : "detail-container"}
          >
            <span className={MaxWindow ? "link-title-1" : "link-title"}>
              {songTitle}
            </span>
            <span className={MaxWindow ? "link-des-1" : "link-des"}>
              {songName}
            </span>
          </div>
        </div>
        <div
          className={
            MaxWindow ? "music-icon-container-1" : "music-icon-container"
          }
        >
