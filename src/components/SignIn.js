...
function SignIn({ setLoggedIn, dispatch, dispatch2 }) {
  ...
  let [stateSignUp1, dispatchSignUp1] = useReducer(
    reducerSignUp1,
    initialStateSignUp1
  );
      ...
      .then((response) => {
        if (!response.ok) {
          if (response.status === 403) {
            dispatchSignUp1({
              type: "handleError",
              errormsg: "Email Id or Password Incorrect",
            });
          } else {
      ...
      <div className="signbox">
        ...
        <form>
          ...
          <input
            required
            className="emailbox"
            value={stateSignUp1.email}
            onChange={(e) =>
              dispatchSignUp1({
                type: "handleEmailInput",
          ...
          <input
            required
            className="passbox"
            value={stateSignUp1.pass}
            onChange={(e) =>
              dispatchSignUp1({ type: "handlePassInput", pass: e.target.value })
            }
            type="password"
          ></input>
          <br></br>
          <br></br>
          <button className="signbtn" onClick={(e) => handleSignIn(e)}>
            Sign in
          </button>
        </form>
        ...
        <div>
          <div className="account">
            ...
            <Link to="/register">
              <p className="createacc">Create your Amazon Account</p>
            </Link>
          </div>
        </div>
      </div>
